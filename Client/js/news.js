...
var NewsManager = {
    ...
    HuggingFace: {
        token: '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',

        // FAST: Single attempt sentiment analysis with immediate fallback
        analyzeSentiment: async function(text) {
            ...
            try {
                const response = await $.ajax({
                    ...
                    method: "POST",
                    headers: {
                        "Authorization": `Bearer ${NewsManager.HuggingFace.token}`,
                ...
                // Immediate fallback to simple detection
                return NewsManager.detectSentimentSimple(text);
            }
        },

        // FAST: Single attempt summarization with immediate fallback
